...
namespace day24_2 {
    class Program {
        ...
        static string Solve(string inputFilename, Int64 boost, bool verbose) {
            ...
            List<UnitGroup> allGroups = new List<UnitGroup>();
            {
                ...
                while ((line = inStream.ReadLine()) != null) {
                    ...
                    var group = new UnitGroup(
                            ...
                            boost + Int64.Parse(m.Groups["attackPower"].Value),
            ...
            while (immuneGroups.Any(g => g.IsAlive) && infectionGroups.Any(g => g.IsAlive)) {
                ...
                {
                    Int64 immuneSystemTotal = immuneGroups.Sum(g => g.NumUnits);
                    Int64 infectionTotal = infectionGroups.Sum(g => g.NumUnits);
                    Console.WriteLine($"{immuneSystemTotal} vs. {infectionTotal}");
                    if (infectionTotal == 79) {
                        verbose = true;
                    }
                }
                if (verbose) {
            ...
            Debug.Assert(infectionUnitsLeft == 0, "Infection won :(");
            return immuneSystemUnitsLeft.ToString();
        }
